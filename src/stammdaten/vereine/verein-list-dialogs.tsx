...
interface EditVereinDialogProps {
    open: boolean
    verein: VereinRowData
    onClose: () => void
    onSave: (v: VereinRowData) => void
    onValidationError: (msg: string) => void
}

export const EditVereinDialog = ({open, verein, onClose, onSave, onValidationError}: EditVereinDialogProps) => {
    const {t} = useTranslation("dashboard")
    const [name, setName] = useState(verein.name)
    const [contact, setContact] = useState<ContactAddress>(verein.contact)
    const [billingSameAsContact, setBillingSameAsContact] = useState(!verein.billingContact)
    const [billing, setBilling] = useState<ContactAddress>(verein.billingContact ?? emptyContact())
    ...
    const handleSave = () => {
        ...
        onSave({
            ...
            name: trimmedName,
            contact: contactDraft,
            ...(billingSameAsContact ? {billingContact: undefined} : {billingContact: {...billing}}),
        })
    }

    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle>{t("stammdaten.vereine-table.edit-verein-title")}</DialogTitle>
            <DialogContent className={styles.dialogContent}>
                <Stack spacing={2} sx={{mt: 1}}>
                    <TextField
                        label={t("stammdaten.vereine-table.columns.verein-name")}
                    ...
                    />
                    <div className={styles.addressSectionTitle}>{t("organisation-admin.contact-address-title")}</div>
                    <ContactFields prefix="edit-kontakt" value={contact} onChange={setContact} />
                    <div className={styles.addressSectionTitle}>{t("organisation-admin.billing-address-title")}</div>
                    <FormControlLabel
                        control={<Checkbox checked={billingSameAsContact} onChange={(e) => setBillingSameAsContact(e.target.checked)} />}
                        label={t("organisation-admin.same-as-contact")}
                    />
                    {!billingSameAsContact ? <ContactFields prefix="edit-rechnung" value={billing} onChange={setBilling} /> : null}
                </Stack>
            </DialogContent>
            <DialogActions>
                <Button onClick={onClose}>{t("stammdaten.vereine-table.cancel")}</Button>
